...
import { FlowActions, fromFlow } from '@euskalfic/shared/flow';
...
import * as fromFlowPage from '../selectors/flow-page.selectors';
  ...
  <ion-content [fullscreen]="true" [scrollEvents]="true" (ionScroll)="logScrolling($any($event))">
    ...
    <div class="container components-background-dark">
      ...
      <ng-container *ngIf="flows$ | async as flows">
        <ng-container *ngIf="flowInfo$ | async as flowInfo">
          <ng-container *ngIf="flowInfo?.status !== EntityStatus.Pending || (componentState?.page || 1) > 1; else loader">
            <ng-container *ngIf="flowInfo?.status !== EntityStatus.Error; else serverError">
              <ng-container *ngIf="(flows?.length || []) > 0; else noData">

                <app-card
                  *ngFor="let item of flows; let i = index; trackBy: trackById"
                  [item]="item"
                  [from]="'flows'"
                ...
                <app-infinite-scroll
                  [slice]="flows.length"
                  [status]="flowInfo?.status || EntityStatus.Loaded"
                  [total]="flowInfo?.total || 0"
                  (loadDataTrigger)="loadData($event, flowInfo?.page || 1)">
                </app-infinite-scroll>
...
export class FlowPage {
  ...
  flowInfo$ = this.store.select(fromFlowPage.selectFlowPageInfo);

  componentState!: componentState;
  trigger = new EventEmitter<componentState>();
  flows$ = this.trigger.pipe(
    concatLatestFrom(() => this.store.select(fromFlow.selectFilterSourceId)),
    tap(([{page, reload, sourceId}, storeSourceId]) => {
      ...
      if(!!reload || page > 1){
        this.store.dispatch(FlowActions.loadFlows({page, sourceId: sourceId || null}));
      }
    }),
    switchMap(() =>
      this.store.select(fromFlow.selectFlows)
    )
  );
